...
def test_valdiate_output():
    ...
    df = pd.DataFrame(data=cols)
    components = generate_components(dtypes=df.dtypes)
    schema = ModelProfile(output_components=components)
    schema.build(output=df)
    components = schema.output_components
...
def test_validate_actual():
    ...
    df = pd.DataFrame(data=cols)
    components = generate_components(dtypes=df.dtypes)
    schema = ModelProfile(actual_components=components)
    schema.build(actual=df)
    components = schema.actual_components
...
def test_multiple_component_types():
    ...
    df = pd.DataFrame(data=cols)
    components = generate_components(dtypes=df.dtypes)
    schema = ModelProfile(input_components=components, output_components=components, actual_components=components)
...
def test_score():
    ...
    profile = ModelProfile(score_components=components)
