...
def load_secret(project_name=None, fpath=None):
    ...
    try:
        return load_secret_file(project_name=project_name, fpath=fpath)
    except Exception as exc:
        print(f"Could not load secret from specific file. ({exc})")
    
    # PRIORITY 1: ENV Variables
    try:
        project_secret = load_secret_env()
        return verify(project_secret)
    except Exception as exc:
    ...
    try:
        return load_secret_file(project_name=project_name, 
                                fpath=Path('./.raymon/secrets.json'))
    except Exception as exc:
    ...
    try:
        return load_secret_file(project_name=project_name,
                                fpath=Path(DEFAULT_FNAME).expanduser())
    except Exception as exc:
