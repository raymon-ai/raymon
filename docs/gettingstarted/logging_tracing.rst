...
In the code snippet below, we create an :class:`raymon.RaymonAPILogger` logger first, and then pass it as a parameter when constructing a :class:`raymon.Trace` object. We do not pass a :code:`trace_id`, which means a :code:`uuid` will be auto generated for this trace. If you already have a :code:`uuid`, you can pass it as :code:`trace_id`.
